...
router.get('/sensors', function(req, res){
    functions.serveSensorData(req, res);
});
...
router.get('/gps', function(req, res){
    functions.serveGPSData(req, res);
});
...
router.get('/images', function(req, res){
    functions.serveImagesInformation(req, res);
});
...
router.get('/images/:docID', function(req, res){
    functions.serveImage(req, res);
});

/**
 * @api {post} /images/:docID Upload image to functions.server
 * @apiName PostImage
 * @apiGroup Images
...
router.post('/images/:docID', upload.single('image'), function(req, res){
    var valid = functions.validateJPEG(req.file.path);
    if(!valid){
        ...
        functions.IandC.sendImageAlert();
        //classifyImage(req.file.originalname, req.file.path, req.body);
        functions.insertImageIntoDatebase(req.file.originalname, req.file.path, req);
    }
});
...
router.delete('/images/:docID', function(req, res){
    functions.deleteImageFromDatabase(req, res);
});
...
router.get('/audio', function(req, res){
    functions.serveAudioInformation(req, res);
});
...
router.post('/audio/:docID', upload.single('audio'), function(req, res){
    var valid = functions.validateAudioFile(req.file.path);
    if(!valid){
        ...
        functions.insertSpeechIntoDatabase(req.file.originalname, req.file.path, req);
    }
});
/**
...
router.param('docID', function(req, res, next, docID){
    if(docID == "latest"){
        if(req.url.indexOf("images") > -1) {
            functions.retrieveLatestImage(res);
        } else if(req.url.indexOf("audio") > -1){
            //functions.retrieveLatestAudio(res);
            res.set('Content-Type', 'audio/x-wav').send(200) //TODo
        }
    } else {
        next();
    }
