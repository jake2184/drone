...
function DroneCoordinator(MQTT, cloudant, sendUpdates){
...
DroneCoordinator.prototype.checkHandlerExistence = function(droneName){
    if(this.handlers[droneName] === undefined){
        ...
        this.handlers[droneName] = new DroneHandler(this.MQTT, this.cloudant, droneName, this.sendUpdates);
    }
};
...
DroneCoordinator.prototype.reset = function(droneName){
    ...
    logger.info("Resetting " + droneName + " to " + this.handlers[droneName].currentMode.name());
    this.sendUpdates(droneName, 'status', this.getStatus(droneName));
};
...
DroneCoordinator.prototype.sendImageAlert = function(droneName, time){

    var data = {"text": "New image uploaded.", "time":time};
    //this.MQTT.sendEvent("node", "server", "image", "json", JSON.stringify(data));
    this.sendUpdates(droneName, 'image', data);
