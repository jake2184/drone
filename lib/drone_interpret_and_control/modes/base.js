...
function ModeBase (cloudant, MQTT, droneName, sendUpdates){
    ...
    this.sendUpdates(this.droneName, "modeChange", {drone: this.droneName, mode: this.name()}, new Date().getTime());
}
...
ModeBase.prototype.sendMavCommand = function(commandName, args){
    ...
    this.MQTT.sendCommand("pi", this.droneName, "mavCommand", "json", JSON.stringify(command), 2);
};
...
ModeBase.prototype.sendPiCommand = function(commandName, args){
  ...
  this.MQTT.sendCommand("pi", this.droneName, "piCommand", "json", JSON.stringify(command), 2);
};
