...
ModeBase.prototype.logEvent = function(events, time, location){
    ...
    this.sendServerEvent("event", JSON.stringify({text: toLog, location: location}));
...
ModeBase.prototype.logSensorReadings = function(sensorReadings){
    ...
    this.cloudant.db.use('positionlog').insert(position, sensorReadings.time.toString(), function(err){
        if(err){
            console.error("[position.insert] Error inserting position reading from time: " + sensorReadings.time);
            console.error("[position.insert] " + err.message);
        }
    });
...
ModeBase.prototype.sendServerEvent = function(type, data){
    this.MQTT.sendEvent("node", "server", type, "json", data);
};
