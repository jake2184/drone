...
const ImageLabelSecondThreshold = 0.7;
...
DroneHandler.prototype.setMode = function(modeType){
	...
	// if (timeNow - this.lastTimeInMode < 3000) {
	// 	logger.info("Mode thrashing?")
	// }

	if (!process.env.TEST && (timeNow - (this.lastTimeInMode[modeType] || 0) < 10000)) {
		logger.info("Not changing " +this.droneName + " to " + modeType + ". Changed from this recently");
		return;
	}

	this.lastTimeInMode[this.currentMode.name()] = timeNow;
	switch(modeType){
		case "Normal":
...
DroneHandler.prototype.determineModeGlobal = function(IandC){
	...
	if (IandC.modeFactors.imageLabels.time < new Date().getTime() - this.oldDataThreshold) {
		delete IandC.modeFactors.imageLabels.time;
		delete IandC.modeFactors.imageLabels.keywords;
	} if(IandC.modeFactors.audioTranscript.time < new Date().getTime() - this.oldDataThreshold){
...
DroneHandler.prototype.processSpeechTranscript = function (transcript, time, location){
	this.modeFactors["audioTranscript"] = {
		'time' : time,
