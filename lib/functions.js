...
function createCloudantDatabases(name){
    ...
    for(var i=0; i<toCreate.length; i++){
        var databaseName = name + toCreate[i];
        cloudant.db.create(databaseName, function(err){
            if(err){
                logger.error("Couldn't create database for drone " + name);
...
function deleteCloudantDatabases(name){
    ...
    for(var i=0; i<toDelete.length; i++){
        var databaseName = name + toDelete[i];
        cloudant.db.destroy(databaseName, function(err){
            if(err){
                logger.error("Couldn't delete database for drone " + name);
                logger.error(err.message);
            } else {
                logger.info("Deleted database for drone " + name);
            }
        })
    }
}
...
function insertDroneCredentials(req, res){
    ...
    dashDB.query(query, [name, model, owner], function(err, response){
        if(err) {
            logger.error("Couldn't insert drone: " + name);
            logger.error(err.message);
            res.status(400).send("Couldn't add drone. Name not unique?");
        } else{
            logger.info("Drone: " + name + " successfully added to database");
            req.session.drones.push(name);
            res.status(200).send("Drone: \"" + name + "\" successfully added to database");
...
function deleteDroneCredentials(req, res){
    ...
    dashDB.query(query, [name], function(err, response){
        if(err) {
            ...
            logger.info("Drone " + name + " successfully deleted from database. Maybe.");
