...
function resetDroneHandler(req, res){
    DroneCoordinator.reset(req.params.dronename);
    res.status(200).send();
}
...
/**
 * Middleware to handle image file upload
...
function handleImageUpload(req, res) {
    ...
    if (!valid) {
        ...
        DroneCoordinator.sendImageAlert(req.params.dronename, req.params.docID);
...
function retrieveLatestAudio(req, res) {
    ...
    audio.list({"descending": true, "include_docs": true, "limit": 2}, function (err, body) {
        if (err) {
            logger.error(err);
            res.status(404).send("Internal error");
        } else {
...
function validateJPEG(filePath){
    try {
        // May cause exception - bad disconnect from client?
...
module.exports = {
    ...
    resetDroneHandler : resetDroneHandler,
    RoleEnum : RoleEnum,
    cloudant : {
