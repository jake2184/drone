...
var profile = {
    logFile :  fs.openSync('public/logs/profile.csv', 'w'),
    init : function(){
        fs.write(this.logFile, 'category,action,fileName,start,duration,extra');
    },
    log : function(string){
        fs.write(this.logFile, string + '\n');
    }
};
profile.init();
/////////////// Load Credentials //////////////
...
var DroneCoordinator = new droneCoordinator(MQTT, cloudant);
...
function sendToSpeechRecognition(req, extension) {
    ...
    speechToText.recognize(params, function(err, results){
        if(err){
            ...
            var speechRecognised = DroneCoordinator.processSpeechTranscript(droneName, transcript, body.time, body.location);
            if(speechRecognised){
    ...
    if (!valid) {
        ...
        images.multipart.insert({name: req.file.originalname, time:req.body.time, location:req.body.location}, attach, req.params.docID, function(err, body) {
            if (err) {
                ...
                profile.log('images,upload,' + req.params.docID + ',' + parseInt(req.body.time) + ',' + duration);
                // Cleanup
    ...
    if(!valid){
        ...
        speechDB.multipart.insert({name: req.file.originalname, time:req.body.time, location:req.body.location}, attach, req.params.docID, function(err, body) {
            if (err) {
                ...
                res.status(200).send("Successfully uploaded audio\n");
                var duration = new Date().getTime() - parseInt(req.body.time);
                profile.log('audio,upload,' + req.params.docID + ',' + parseInt(req.body.time) + ',' + duration);
            }
        });
        ...
        function testSendToSpeechRecognition(req, extension) {
            ...
            speechToText.recognize(params, function(err, results){
                if(err){
                    ...
                    var duration = new Date().getTime() - parseInt(req.body.time);
                    profile.log('audio,upload,' + req.params.docID + ',' + parseInt(req.body.time) + ',' + duration + ',' + response.confidence + ',' + response.transcript);
                }
            });
        }
        if(audiofilePath.endsWith(".mp3")) {
