...
var profile2 = {
    ...
    writeImages : function(){
        ...
        for (file in data) {
            if (data.hasOwnProperty(file)) {
                fs.writeSync(this.imageFile, JSON.stringify(data[file].labels));
            }
        }
    ...
    writeAudio : function(){
        var data = this.audio;
        fs.writeSync(this.audioFile, 'filename,upload,database,recognition\n');
        for (var file in data) {
            if (data.hasOwnProperty(file)) {
...
function retrieveLatestImage(req, res){
    ...
    var images = cloudant.use(req.params.dronename + '_images');
    images.list({startkey:'0', "descending": true, "include_docs": true, "limit": 2}, function(err, body){
        if(err){
            logger.error(err);
