...
function serveDronesInformation(req, res){
    ...
    dashDB.query(query, [username], function(err, response){
        if(err) {
            logger.error("Couldn't get drone information for user: " + username);
            logger.error(err.message);
            res.status(404).send("Couldn't get data");
            return false;
        } else{
            logger.info("Drone data for " + username + " successfully retrieved");
...
function deleteDroneCredentials(req, res){
    ...
    dashDB.query(query, [name], function(err, response){
        if(err) {
            logger.error("Couldn't delete drone: " + name);
            logger.error(err.message);
